...
describe("ProfileCard", () => {
  describe("Layout", () => {
    ...
    it("displays default image when user doesn not have one", () => {
    ...
    it("does not displays edit button when isEditable not provided", () => {
      const { queryByText } = render(<ProfileCard user={user} />);
      const editButton = queryByText("Edit");
    ...
    it("dispalys label for displayName in edit mode", () => {
      const { container } = render(
        <ProfileCard user={user} inEditMode={true} />
      );
